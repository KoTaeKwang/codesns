...
public class bbsDAO {
	...
	public int insertBbs(bbsDTO dto){  //게시판 추가
		int result=0;
		PreparedStatement pstmt=null;
		 StringBuffer sb = new StringBuffer();
		 try{
			 
			 sb.append("INSERT INTO bbs (num, subject, userid, content, imagefilename)");
...
public List<bbsDTO> ListBbs(){   //전체 리스트 보여주기
...
public List<bbsDTO> ListSearchBbs(String searchValue)  //검색리스트 보여주기
{
	List<bbsDTO> listName = new ArrayList<>();
	 PreparedStatement pstmt=null;
	 StringBuffer sb = new StringBuffer();
	 ResultSet rs = null;
	 
	 try {
		 sb.append(" SELECT num, subject, userid, created, content, liked, dislike, imagefilename FROM bbs WHERE ");
...
public int updateLike(int num, String userid){   //LIKE 올리기
	...
	try {
		sql = null;
		sql = " INSERT INTO bbs_liked(num, userid) VALUES(?, ?) ";
		...
		sql = null;
		sql = " UPDATE bbs SET liked=liked+1 WHERE num = ? ";
		pstmt=conn.prepareStatement(sql);
		pstmt.setInt(1, num);
...
public int updateDislike(int num, String userid){  //DisLike 올리기
	...
	try {
		sql = null;
		sql = " INSERT INTO bbs_dislike(num, userid) VALUES(?, ?) ";
		pstmt = conn.prepareStatement(sql);
		pstmt.setInt(1, num);
		pstmt.setString(2, userid);
		...
		sql = null;
		sql = "UPDATE bbs SET dislike = dislike + 1 WHERE num = ?";
		pstmt = conn.prepareStatement(sql);
		pstmt.setInt(1, num);
...
public bbsDTO readBbs(int num){   //해당 게시판 
	bbsDTO dto=null;
	PreparedStatement pstmt=null;
	ResultSet rs=null;
	StringBuffer sb=new StringBuffer();
...
public int dataCount(String searchValue) {
	...
	try {
		sql = "SELECT NVL(COUNT(*), 0) FROM bbs WHERE userid || subject || content LIKE '%' || ? || '%' ";
...
public List<bbsDTO> listBbs(int start, int end, String searchValue) {
	List<bbsDTO> list = new ArrayList<>();
	...
	try {
		...
		sb.append(" SELECT ROWNUM rnum, tb.* FROM (");
		sb.append("  SELECT c.num, c.subject, c.content, c.liked, c.dislike, c.created, c.imagefilename, c.userid, m.profile");
		sb.append("  FROM bbs c JOIN member m ON c.userId=m.userId WHERE c.userid || c.subject || c.content LIKE '%' || ? || '%' ");	
		sb.append("  ORDER BY num DESC");
		sb.append(" ) tb WHERE ROWNUM <= ?");
		sb.append(") WHERE rnum >= ?");
