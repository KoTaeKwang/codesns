...
function listPage(page) {
   ...
   location.href="<%=cp%>/bbs/list.do?"+params;
}


function like(num) {
   location.href="<%=cp %>/bbs/updateLike.do?translatedText=${translatedText}&num=" + num+"&scrollHeight="+scrollHeight+"&searchValue=${searchValue}";
}

function dislike(num) {
   location.href="<%=cp%>/bbs/updateDislike.do?translatedText=${translatedText}&num=" + num+"&scrollHeight="+scrollHeight+"&searchValue=${searchValue}";
}

function bbstransgo(num){
	var content=document.getElementById("memo"+num).value;

   location.href="<%=cp%>/bbs/change.do?num="+num+"&content="+content+"&country=${sessionScope.member.country}&searchValue=${searchValue}&scrollHeight="+scrollHeight+"";
}

function moveYourPage(userid) {
      location.href="<%=cp %>/member/yourpage.do?userid=" + userid + "&scrollHeight=" + scrollHeight+ "&beforepage=bbs";
   }

function bbscommentOk(num) {
    ...
    f.action = "<%=cp%>/bbs/comment.do?translatedText=${translatedText}&scrollHeight="+scrollHeight+"&searchValue=${searchValue}";
    f.submit();
}

function bbs_comment_delete(num,comment_num) {
   location.href="<%=cp%>/bbs/commemt_delete.do?translatedText=${translatedText}&num=" + num+"&scrollHeight="+scrollHeight+"&comment_num="+comment_num+"&searchValue=${searchValue}";
}
...
<div class="div" style=" height: 500px;">
   <img src="<%=cp%>/uploads/bbs/${dto.imagefilename}" width="599px" height="499px">
</div>
</c:if>
...
<%-- <input class="btn btn-primary" type="button" value="번역" name="translate" onclick="bbstransgo('${dto.num}','${dto.content}');"> --%>
<button type="button" class="btn btn-info btn-sm"name="translate" onclick="bbstransgo('${dto.num}');">
...
<div class="div" id="${dto.num}" style="border-bottom:1px solid #EDEDED;"><pre style="background: #FFFFFF; background-color: transparent; border: 0">${dto.content}</pre></div>
</c:if>

<c:if test="${dto.num == num}">
<div class="div"  id="${dto.num}" style="border-bottom:1px solid #EDEDED;"><pre style="background: #FFFFFF; background-color: transparent; border: 0">${translatedText}</pre></div>
</c:if>
...
<table style="width: 596px; border-right: 1px solid #EDEDED; margin: 0px auto; border-spacing: 0px; table-layout: fixed">      
            <tr align="center" height="0"> 
               <td width="105" style=" border-bottom: 1px solid #EDEDED;"></td>
               <td width="380" style=" border-bottom: 1px solid #EDEDED;"></td>
               <td width="110" style=" border-bottom: 1px solid #EDEDED; "></td>
           </tr>
      ...
      <c:if test="${dto.num==cdto.num}">
           ...
           <tr align="center" bgcolor="#ffffff" height="25" >
              ...
              <c:if test="${sessionScope.member.userId==cdto.userid}">
               ...
               <a style="cursor: pointer;" onclick="bbs_comment_delete('${dto.num}','${cdto.comment_num}')">
...
<%--                <input type="button" class="btn btn-primary" onclick="bbscommentOk('${dto.num}');" value="등록"> --%>
                <button type="button" class="btn btn-warning btn-sm" onclick="bbscommentOk('${dto.num}');">
